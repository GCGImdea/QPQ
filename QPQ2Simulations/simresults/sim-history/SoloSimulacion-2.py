...
def doSimulation(numplayers, numLiars, numclusters, betaf, thresholdFunct, historyLen):
    ...
    for k in range(numberSimulations):
        ...
        for version in [1,2]:
            ...
            simresults['UtilityHonest'].append(utT[numLiars:].mean())
            simresults['FNTotalHonest'].append(fnT[numLiars:].mean())
            if (numLiars > 0):
                simresults['UtilityDishonest'].append(utT[0:numLiars].mean())
                simresults['FNTotalDishonest'].append(fnT[0:numLiars].mean())
            else:
...
# History length
historyLenArray = [100, 300, 1000, 3000, 10000] 
alpha = 1 
